let menu = [
  ...
  {category:'Healthy Bowls',slug:'healthy-bowls',tagline:'Balanced fuel · bold flavour',items:[['Acai Bowl','Açaí topped with banana, berries, granola and peanut butter',46,'assets/bowl-acai-new.jpg','VG'],['Blue Crash Bowl','A vibrant blue smoothie bowl with mango, banana, berries and granola',44,'assets/bowl-blue-crash-new.jpg','VG'],['Mystic Matcha Bowl','A berry smoothie bowl with avocado, banana, cucumber and granola',44,'assets/bowl-mystic-matcha-new.jpg','VG'],['Green Dream Bowl','A colourful wellness bowl with fruit, vegetables, nuts and granola',44,'assets/bowl-green-dream-new.jpg','VG'],['Detox Me Later Glow Bowl','A pink smoothie bowl topped with tropical fruit, berries and pomegranate',44,'assets/bowl-detox-glow-new.jpg','VG'],['Tropical Glow Bowl','A tropical green bowl with avocado, mango, pineapple and berries',44,'assets/bowl-tropical-glow-new.jpg','VG'],['Watermelon Feta','Fresh watermelon, feta, chia seeds and mint',25,'assets/bowl-watermelon-feta-new.jpg','V']]},
...
customizeModal.innerHTML=`<button class="customize-close close-btn" type="button" aria-label="Close drink customization">×</button><div class="modal-header customize-header"><span class="customize-eyebrow eyebrow">MAKE IT YOURS</span><h2 id="customize-title">Customize Your Drink</h2><h3 class="customize-product-name product-name"></h3></div><form><div class="customize-grid customize-content"><section class="custom-section coffee-section"><h4>COFFEE BEANS</h4><div class="customize-options option-grid coffee-options">${pricedOptions(beanOptions,'beans')}</div></section><section class="custom-section milk-section"><h4>MILKS</h4><div class="customize-options option-grid milk-options">${pricedOptions(milkOptions,'milk')}</div></section><section class="custom-section add-on-section"><h4>SYRUPS</h4><div class="customize-options option-grid addon-options">${pricedOptions(syrupOptions,'syrup')}</div></section><section class="custom-section customize-comment comment-section"><h4>COMMENT / SPECIAL INSTRUCTIONS</h4><p>Tell us what you’d like to reduce, remove, or add to this drink.</p><textarea id="customize-instructions" name="instructions" maxlength="500" rows="4" placeholder="Add your comment or special instructions..."></textarea></section><div class="bottom-options"><section class="custom-section temperature-section"><h4>HOT / ICE</h4><div class="customize-options option-grid temperature-options">${pricedOptions([['Hot',0],['Ice',0]],'temperature')}</div></section><section class="custom-section service-section"><h4>DINE IN / DELIVERY</h4><div class="customize-options option-grid service-options">${pricedOptions([['Dine-in',0],['Delivery',6]],'service')}</div></section></div></div><p class="customize-note" aria-live="polite"></p><button class="customize-save save-btn" type="submit">SAVE SELECTION</button></form>`;
...
const customizationExtra=customization=>Number(customization?.extraPrice||0);
const customizationText=customization=>{if(!customization)return'';if(!customization.beans)return`Coffee: ${customization.coffee||'Standard'}; Milk: ${customization.milk||'Standard'}; Add-ons: ${customization.addons?.length?customization.addons.join(', '):'None'}`;return`Beans: ${customization.beans}; Milk: ${customization.milk}; Syrup: ${customization.syrup}; ${customization.temperature}; ${customization.service}${customizationExtra(customization)?`; Additions: + AED ${customizationExtra(customization).toFixed(2)}`:''}`};
const updateCustomizationPrice=()=>{const extra=[...customizeModal.querySelectorAll('input:checked')].reduce((sum,input)=>sum+Number(input.dataset.price||0),0);customizeModal.querySelector('.save-btn').textContent=`SAVE SELECTION${extra?` · + AED ${extra.toFixed(2)}`:''}`;return extra};
document.addEventListener('click',event=>{
  const button=event.target.closest('.customize-product');if(!button)return;
  customizeProduct=button.dataset.product;
  const defaults={beans:'Brazil',milk:'Fresh Milk',syrup:'None',temperature:'Hot',service:'Dine-in'},saved=readCustomizations()[customizeProduct]||defaults;
...
customizeModal.querySelector('form').addEventListener('submit',event=>{
  ...
  const customization={beans:form.get('beans'),milk:form.get('milk'),syrup:form.get('syrup'),temperature:form.get('temperature'),service:form.get('service'),extraPrice:updateCustomizationPrice()},instructions=String(form.get('instructions')||'').trim();
