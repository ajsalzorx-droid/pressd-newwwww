...
content.innerHTML=menu.map((c,index)=>c.editorial?`<article class="menu-category editorial ${index%2?'flip':''}" id="${c.slug}" data-category="${c.slug}"><div class="editorial-image"><img src="${c.items[0][3]}" alt="${c.category}" loading="lazy"></div><div class="editorial-list"><header><p>${String(index+1).padStart(2,'0')} · MENU</p><h3>${c.category}</h3><span>${c.tagline}</span></header>${c.items.map((x,i)=>`<div class="line-item"><b>${String(i+1).padStart(2,'0')}</b><div class="product-summary"><h4>${x[0]} ${x[4]?`<i>${x[4]}</i>`:''}</h4><button class="view-details" data-category-index="${index}" data-item-index="${i}">View Details <span aria-hidden="true">↗</span></button></div><div class="line-actions"><strong>AED ${x[2]}</strong><button class="add-cart" data-name="${x[0]}" data-price="${x[2]}" data-image="${x[3]}">Add +</button></div></div>`).join('')}</div></article>`:`<article class="menu-category" id="${c.slug}" data-category="${c.slug}"><header class="category-title"><div><p>${String(index+1).padStart(2,'0')} · MENU</p><h3>${c.category}</h3></div><span>${c.tagline}</span></header><div class="menu-grid">${c.items.map((x,i)=>`<div class="menu-card"><div class="menu-img"><img src="${x[3]}" alt="${x[0]}" loading="lazy">${x[4]?`<span>${x[4]}</span>`:''}</div><div class="menu-card-info"><div class="product-summary"><h4>${x[0]}</h4><button class="view-details" data-category-index="${index}" data-item-index="${i}">View Details <span aria-hidden="true">↗</span></button></div><div class="menu-card-actions"><strong>AED ${x[2]}</strong><button class="add-cart" data-name="${x[0]}" data-price="${x[2]}" data-image="${x[3]}">Add +</button></div></div></div>`).join('')}</div></article>`).join('');
...
const cartImageFor=item=>item.image||menuItemByName.get(item.name)?.[3]||'';
const orderText=()=>{const total=cart.reduce((sum,item)=>sum+item.price*item.qty,0),lines=cart.map(item=>`• ${item.name} x${item.qty} — AED ${item.price*item.qty}`);return `Hello PRESS'D! I'd like to order:\n\n${lines.join('\n')}\n\nTotal: AED ${total}\n\nPlease confirm availability and delivery/pickup details.`};
let preparedOrderFile=null,preparedOrderSignature='';
const loadOrderImage=src=>new Promise(resolve=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>resolve(null);image.src=src});
const buildOrderShareImage=async()=>{
  const width=1080,rowHeight=180,height=270+cart.length*rowHeight,canvas=document.createElement('canvas'),context=canvas.getContext('2d');canvas.width=width;canvas.height=height;
  context.fillStyle='#0b0c0b';context.fillRect(0,0,width,height);context.fillStyle='#ffb419';context.fillRect(0,0,width,16);context.fillStyle='#fff8ec';context.font='700 64px Arial';context.fillText("PRESS'D",64,102);context.fillStyle='#ffb419';context.font='600 26px Arial';context.fillText('WHATSAPP ORDER',66,146);
  const images=await Promise.all(cart.map(item=>loadOrderImage(cartImageFor(item))));
  cart.forEach((item,index)=>{const y=205+index*rowHeight,image=images[index];context.fillStyle='#171817';context.beginPath();context.roundRect(44,y-22,width-88,rowHeight-18,24);context.fill();if(image){const boxX=64,boxY=y-4,boxW=150,boxH=120,scale=Math.max(boxW/image.width,boxH/image.height),sourceW=boxW/scale,sourceH=boxH/scale;context.save();context.beginPath();context.roundRect(boxX,boxY,boxW,boxH,16);context.clip();context.drawImage(image,(image.width-sourceW)/2,(image.height-sourceH)/2,sourceW,sourceH,boxX,boxY,boxW,boxH);context.restore()}context.fillStyle='#fff8ec';context.font='700 34px Arial';context.fillText(item.name,244,y+38,590);context.fillStyle='#bbb5aa';context.font='500 26px Arial';context.fillText(`Quantity: ${item.qty}`,244,y+84);context.fillStyle='#ffb419';context.font='700 30px Arial';context.textAlign='right';context.fillText(`AED ${item.price*item.qty}`,1010,y+60);context.textAlign='left'});
  const total=cart.reduce((sum,item)=>sum+item.price*item.qty,0);context.fillStyle='#fff8ec';context.font='700 38px Arial';context.fillText('TOTAL',64,height-42);context.fillStyle='#ffb419';context.textAlign='right';context.fillText(`AED ${total}`,1016,height-42);context.textAlign='left';
  const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',.88));return blob?new File([blob],'pressd-order.jpg',{type:'image/jpeg'}):null;
};
const prepareOrderShare=()=>{const signature=JSON.stringify(cart.map(({name,price,qty,image})=>({name,price,qty,image})));preparedOrderFile=null;preparedOrderSignature='';if(!cart.length)return;buildOrderShareImage().then(file=>{if(file&&signature===JSON.stringify(cart.map(({name,price,qty,image})=>({name,price,qty,image})))){preparedOrderFile=file;preparedOrderSignature=signature}})};
const fileAsDataUrl=file=>new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(reader.error);reader.readAsDataURL(file)});
const shareOrderFallback=()=>{if(preparedOrderFile&&navigator.share&&(!navigator.canShare||navigator.canShare({files:[preparedOrderFile]})))return navigator.share({title:"PRESS'D Order",text:orderText(),files:[preparedOrderFile]});location.href=whatsappOrder.href;return Promise.resolve()};
const deliverWhatsAppOrder=async()=>{const response=await fetch('/api/whatsapp-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:orderText(),image:await fileAsDataUrl(preparedOrderFile)})});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||'Unable to send order');return result};
let cartToastTimer;
...
const renderCart=()=>{
  ...
  cartItems.innerHTML=cart.map((item,index)=>`<article class="cart-item"><img class="cart-item-image" src="${cartImageFor(item)}" alt="${item.name}"><div class="cart-item-copy"><h3>${item.name}</h3><p>AED ${item.price}</p></div><div class="cart-quantity"><button data-cart-action="minus" data-index="${index}" aria-label="Remove one ${item.name}">−</button><span>${item.qty}</span><button data-cart-action="plus" data-index="${index}" aria-label="Add one ${item.name}">+</button></div></article>`).join('');
  if(cart.length){whatsappOrder.href=`https://wa.me/971543962660?text=${encodeURIComponent(orderText())}`;whatsappOrder.classList.remove('disabled');whatsappOrder.setAttribute('aria-disabled','false')}else{whatsappOrder.href='#';whatsappOrder.classList.add('disabled');whatsappOrder.setAttribute('aria-disabled','true')}
  saveCart();
  prepareOrderShare();
};
document.addEventListener('click',event=>{const add=event.target.closest('.add-cart');if(add){const name=add.dataset.name,price=Number(add.dataset.price),image=add.dataset.image,existing=cart.find(item=>item.name===name);if(existing){existing.qty++;existing.image=image||existing.image}else cart.push({name,price,image,qty:1});renderCart();animateCartAdd(add,name);add.textContent='Added ✓';setTimeout(()=>{add.textContent='Add +';add.classList.remove('is-added')},1200)}const control=event.target.closest('[data-cart-action]');if(control){const index=Number(control.dataset.index);if(control.dataset.cartAction==='plus')cart[index].qty++;else cart[index].qty--;cart=cart.filter(item=>item.qty>0);renderCart()}});
cartToggle.addEventListener('click',openCart);dockCart.addEventListener('click',openCart);cartClose.addEventListener('click',closeCart);cartBackdrop.addEventListener('click',closeCart);document.addEventListener('keydown',event=>{if(event.key==='Escape'&&cartDrawer.classList.contains('open'))closeCart()});whatsappOrder.addEventListener('click',event=>{if(!cart.length){event.preventDefault();return}const signature=JSON.stringify(cart.map(({name,price,qty,image})=>({name,price,qty,image})));if(!preparedOrderFile||preparedOrderSignature!==signature)return;event.preventDefault();const original=whatsappOrder.textContent;whatsappOrder.textContent='Sending order…';whatsappOrder.classList.add('is-sending');deliverWhatsAppOrder().then(()=>{whatsappOrder.textContent='Order sent ✓';setTimeout(()=>{whatsappOrder.textContent=original;whatsappOrder.classList.remove('is-sending')},2600)}).catch(()=>{whatsappOrder.textContent=original;whatsappOrder.classList.remove('is-sending');shareOrderFallback().catch(error=>{if(error.name!=='AbortError')location.href=whatsappOrder.href})})});renderCart();
