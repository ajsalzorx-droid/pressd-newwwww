...
const buildOrderShareImage=async()=>{
  ...
  const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png',.92));return blob?new File([blob],'pressd-order.png',{type:'image/png'}):null;
};
...
cartToggle.addEventListener('click',openCart);dockCart.addEventListener('click',openCart);cartClose.addEventListener('click',closeCart);cartBackdrop.addEventListener('click',closeCart);document.addEventListener('keydown',event=>{if(event.key==='Escape'&&cartDrawer.classList.contains('open'))closeCart()});whatsappOrder.addEventListener('click',event=>{if(!cart.length){event.preventDefault();return}const signature=JSON.stringify(cart.map(({name,price,qty,image})=>({name,price,qty,image})));if(preparedOrderFile&&preparedOrderSignature===signature&&navigator.share&&(!navigator.canShare||navigator.canShare({files:[preparedOrderFile]}))){event.preventDefault();navigator.share({title:"PRESS'D Order",text:orderText(),files:[preparedOrderFile]}).catch(error=>{if(error.name!=='AbortError')location.href=whatsappOrder.href})}});renderCart();
